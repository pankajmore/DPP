/** https://leetcode.com/problems/target-sum/ Created by pankaj on 21/01/17. */
public class TargetSum {
  public int findTargetSumWays(int[] nums, int S) {
    ...
    for (int i = 1; i < N; i++) {
      for (int j = -max; j <= max; j++) {
        dp[i][j + max] =
            (j - nums[i] + max < 0 ? 0 : dp[i - 1][j - nums[i] + max])
  ...
  private int waysHelper(int[] nums, int i, int S, int curr) {
    ...
    return waysHelper(nums, i + 1, S, curr - nums[i]) + waysHelper(nums, i + 1, S, curr + nums[i]);
  }
  ...
  private int subsetSum(int[] nums, int s) {
    ...
    for (int i = 1; i <= N; i++) {
      for (int j = 0; j <= s; j++) {
        dp[i % 2][j] =
            dp[(i - 1) % 2][j] + (j < nums[i - 1] ? 0 : dp[(i - 1) % 2][j - nums[i - 1]]);
      }
    }
    return dp[N % 2][s];
  }
