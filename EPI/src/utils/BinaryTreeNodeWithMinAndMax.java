...
public class BinaryTreeNodeWithMinAndMax {
  ...
  public BinaryTreeNodeWithMinAndMax(
      int val, BinaryTreeNodeWithMinAndMax left, BinaryTreeNodeWithMinAndMax right) {
    this._val = val;
    this._left = left;
    this._right = right;
    _min = _max = this;
    if (left != null) {
      if (this.left().min().val() < _min.val()) _min = this.left().min();
      if (this.left().max().val() > _max.val()) _max = this.left().max();
    }
    if (right != null) {
      if (this.right().min().val() < _min.val()) _min = this.right().min();
      if (this.right().max().val() > _max.val()) _max = this.right().max();
    }
  }
