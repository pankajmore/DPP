...
public class BinaryTreeNode {
  ...
  public static BinaryTreeNode construct(List<Integer> inOrder, List<Integer> otherOrder)
      throws Exception {
    Map<Integer, Integer> nodeToIndex = new HashMap<>();
    for (int i = 0; i < inOrder.size(); i++) nodeToIndex.put(inOrder.get(i), i);
    if (inOrder.get(0).equals(otherOrder.get(0))) //postOrder
    return constructFromInOrderAndPostOrder(
          otherOrder, 0, otherOrder.size(), 0, inOrder.size(), nodeToIndex);
    return constructFromInOrderAndPreOrder(
        otherOrder, 0, otherOrder.size(), 0, inOrder.size(), nodeToIndex);
  }

  private static BinaryTreeNode constructFromInOrderAndPostOrder(
      List<Integer> postOrder,
      Integer postOrderStart,
      Integer postOrderEnd,
    ...
    root.left =
        constructFromInOrderAndPostOrder(
            postOrder,
            postOrderStart,
  ...
  private static BinaryTreeNode constructFromInOrderAndPreOrder(
    ...
    root.left =
        constructFromInOrderAndPreOrder(
            preOrder,
            preOrderStart + 1,
  ...
  public int height() {
    ...
    if (right() != null && right().height() > h) h = right().height();
    return 1 + h;
  }
  ...
  public BinaryTreeNode min() {
    ...
    if (this.left() != null && this.left().min().val() < m.val()) m = this.left().min();
    if (this.right() != null && this.right().min().val() < m.val()) m = this.right().min();
    return m;
  }

  public BinaryTreeNode max() {
    BinaryTreeNode m = this;
    if (this.left() != null && this.left().max().val() > m.val()) m = this.left().max();
    if (this.right() != null && this.right().max().val() > m.val()) m = this.right().max();
    return m;
  }
  ...
  public List<BinaryTreeNode> inOrder() {
    ...
    if (this.left() != null) ls.addAll(this.left().inOrder());
    ls.add(this);
    if (this.right() != null) ls.addAll(this.right().inOrder());
    return ls;
  }
  ...
  public List<Integer> inOrderVals() {
    ...
    if (this.left() != null) vals.addAll(this.left().inOrderVals());
    vals.add(this.val());
    if (this.right() != null) vals.addAll(this.right().inOrderVals());
    return vals;
  }
  ...
  public List<BinaryTreeNode> preOrder() {
    List<BinaryTreeNode> ls = new ArrayList<>();
    ls.add(this);
    if (this.left() != null) ls.addAll(this.left().preOrder());
    if (this.right() != null) ls.addAll(this.right().preOrder());
    return ls;
  }
  ...
  public List<BinaryTreeNode> postOrder() {
    List<BinaryTreeNode> ls = new ArrayList<>();
    if (this.left() != null) ls.addAll(this.left().postOrder());
    if (this.right() != null) ls.addAll(this.right().postOrder());
    ls.add(this);
    return ls;
  }
  ...
  public Pair<Pair<Integer, Integer>, Boolean> isBSTPostOrderImpl() {
    ...
    if (this.left() != null) {
      ...
      if (!l.second() || this.val() < l.first().second())
        return new Pair<>(new Pair<>(0, 0), false);
   ...
   * @param y another node in this BST assume x.val() < y.val(), x and y are in the bST
  ...
  public boolean equals(Object other) {
    ...
    if (this.left != null) ok &= this.left.equals(btOther.left);
    if (this.right != null) ok &= this.right.equals(btOther.right);
    return ok;
  }
}
