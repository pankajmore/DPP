...
public class ThreadedBinaryTree {
  ...
  private static ThreadedBinaryTree copy(@NotNull BinaryTreeNode n) {
    ...
    if (n.left != null) c.left = copy(n.left);
    if (n.right != null) c.right = copy(n.right);
    return c;
  }

  private List<ThreadedBinaryTree> inOrderPreThreading() {
    List<ThreadedBinaryTree> nodes = new ArrayList<>();
    if (this.left != null) nodes.addAll(this.left.inOrderPreThreading());
    nodes.add(this);
    if (this.right != null) nodes.addAll(this.right.inOrderPreThreading());
    return nodes;
  }

  public List<ThreadedBinaryTree> inOrder() {
    List<ThreadedBinaryTree> nodes = new ArrayList<>();
    if (!this.leftThreaded && this.left != null) nodes.addAll(this.left.inOrder());
    nodes.add(this);
    if (!this.rightThreaded && this.right != null) nodes.addAll(this.right.inOrder());
    return nodes;
  }
