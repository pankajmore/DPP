...
public class BinaryTreeNodeWithParent {
  ...
  public BinaryTreeNodeWithParent(
      int val, BinaryTreeNodeWithParent left, BinaryTreeNodeWithParent right) {
    this._val = val;
    this._left = left;
    this._right = right;
    this._parent = null;
  ...
  public BinaryTreeNodeWithParent preOrderSuccessor() {
    ...
    while (n._parent != null) {
      if (n.isLeftChild() && n._parent._right != null) return n._parent._right;
      n = n._parent;
    }
    return null;
  }
  ...
  public BinaryTreeNodeWithParent postOrderPredecessor() {
    ...
    if (n._left != null) return n._left;
    while (n._parent != null) {
      if (n.isRightChild() && n._parent._left != null) return n._parent._left;
      n = n._parent;
    }
    return null;
  }
  ...
  /**
   * Time : O(max(d1,d2)) Space : O(1)
   *
   * @param n1 node 1
   * @param n2 node 2
   * @return the lca of n1 and n2
   */
  public BinaryTreeNodeWithParent leastCommonAncestor(
      BinaryTreeNodeWithParent n1, BinaryTreeNodeWithParent n2) {
    int d1 = n1.depth();
    int d2 = n2.depth();
    int t = abs(d1 - d2);
  ...
  /**
   * Time: O(max(d(lca-n1),d(lca-n2)) Space: O(max(d(lca-n1),d(lca-n2))
   *
   * @param n1 node 1
   * @param n2 node 2
   * @return lca of n1 and n2
   */
  public BinaryTreeNodeWithParent leastCommonAncestorHashTable(
      BinaryTreeNodeWithParent n1, BinaryTreeNodeWithParent n2) {
