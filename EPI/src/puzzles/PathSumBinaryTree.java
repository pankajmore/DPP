...
public class PathSumBinaryTree {
  public static boolean leafWithPathSum(BinaryTreeNode root, int sum) {
    ...
    return leafWithPathSum(root.left, sum - root.val())
        || leafWithPathSum(root.right, sum - root.val());
  }
  ...
  private static boolean pathWithSumHelper(
      BinaryTreeNode root, int sum, List<BinaryTreeNode> path) {
    if (root == null) return false;
    path.add(root);
    sum -= root.val();
    if ((root.left == null && root.right == null && sum == 0)
        || pathWithSumHelper(root.left, sum, path)
        || pathWithSumHelper(root.right, sum, path)) return true;
  ...
  private static void allPathsWithSumHelper(
      BinaryTreeNode root,
      int sum,
      List<BinaryTreeNode> path,
      List<List<BinaryTreeNode>> allPaths) {
    if (root == null) return;
    path.add(root);
    sum -= root.val();
