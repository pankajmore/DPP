...
public class RebuildBSTPreOrder {
  static int i;
  /**
   * Builds a BST given its preOrder traversal. Assumes that the keys in the BST are unique. If the
   * keys are not unique, there exist multiple BSTs that correspond to the same preOrder sequence
  ...
  public static BinaryTreeNode rebuildBSTFromPreOrder(List<Integer> preOrderSequence) {
    ...
    for (i = 0; i < preOrderSequence.size(); i++)
      if (preOrderSequence.get(i) > preOrderSequence.get(0)) break;
    return new BinaryTreeNode(
        preOrderSequence.get(0),
        rebuildBSTFromPreOrder(preOrderSequence.subList(1, i)),
  ...
  private static BinaryTreeNode rebuildBSTFromPreOrderFastHelper(
      List<Integer> preOrderSequence, int minValue, int maxValue) {
    ...
    return new BinaryTreeNode(
        val,
        rebuildBSTFromPreOrderFastHelper(preOrderSequence, minValue, val),
