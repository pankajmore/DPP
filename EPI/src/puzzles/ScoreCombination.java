...
public class ScoreCombination {
  /**
   * DP optimised solution Time complexity : O(s*n) where n = scores.size() Space Complexity : O(s)
   *
   * @param s the score s
  ...
  /**
   * Time Complexity : Exponential Space Complexity : O(s/Scores_min)
   *
  ...
  /**
   * Time Complexity : O(sn) Space Complexity : O(s)
   *
   * @param s the score to achieve
   * @param scores the list of scores
   * @return number of permutations in which s can be scored
  ...
  public static int countPermutationsDP(int s, List<Integer> scores) {
    ...
    A[0] = 1;
    for (int i = 1; i <= s; i++) {
      for (Integer score : scores) A[i] += i - score >= 0 ? A[i - score] : 0;
    }
    return A[s];
  }
