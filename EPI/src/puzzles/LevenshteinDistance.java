...
public class LevenshteinDistance {
  ...
  /**
   * Edit Distance between s1 of length m and s2 or length n Time : O(m*n) Space : O(min(m,n))
   *
   * @param s1
   * @param s2
   * @return
      ...
      for (int j = 1; j <= n; j++) {
        ...
        A[j] =
            s1.charAt(i - 1) == s2.charAt(j - 1)
                ? prev
                : Math.min(1 + Math.min(A[j - 1], A[j]), prev + costEdit);
        prev = pp;
      }
    }
    return A[n];
  ...
  /**
   * Compute the number of inserts + deletes + edits needed to transform s1 to s2
   *
   * @param s1
   * @param s2
   * @return
  ...
  public static int editDistanceRecursive(String s1, String s2) {
    ...
    return Math.min(
        Math.min(
            editDistance(s1.substring(0, s1.length() - 1), s2) + 1,
            editDistance(s1, s2.substring(0, s2.length() - 1)) + 1),
        editDistance(s1.substring(0, s1.length() - 1), s2.substring(0, s2.length() - 1))
