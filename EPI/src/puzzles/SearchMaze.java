...
public class SearchMaze {
    public static List<Point> searchMaze(List<List<Integer>> maze, Point start, Point stop) {
        ...
        path.addFirst(start);
        if (!searchMazeHelper(maze, start, stop, path))
            path.removeLast();
        return path;
    }

    private static boolean searchMazeHelper(List<List<Integer>> maze, Point curr, Point stop, LinkedList<Point> path) {
        if (curr.equals(stop)) return true;
        for (Point child : children(curr, maze)) {
            maze.get(child.x).set(child.y, 1);
            path.add(child);
            if (searchMazeHelper(maze, child, stop, path))
                return true;
            path.removeLast();
        }
        return false;
    ...
    private static boolean isFeasible(Point point, List<List<Integer>> maze) {
        return point.x >= 0 && point.x < maze.size() && point.y >= 0 && point.y < maze.get(point.x).size()
                && maze.get(point.x).get(point.y) == 0;
    }

    public static boolean checkPath(List<Point> path, List<List<Integer>> maze, Point start, Point stop) {
        if (path.isEmpty()) return true;
        if (!path.get(0).equals(start)) return false;
        for (int i = 1; i < path.size(); i++) {
            Point prev = path.get(i - 1);
            Point curr = path.get(i);
            if (!children(prev, maze).contains(curr))
                return false;
        }
