...
public class NQueens {
  ...
  private static void nQueens(List<String> grid, int n, List<ChessGrid> grids) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (valid(newGrid)) nQueens(newGrid, n, grids);
    }
  }
  ...
  private static boolean valid(List<String> grid) {
    ...
    int c = y, d = y;
    for (int row = 0; row < x; row++) if (grid.get(row).charAt(y) == 'Q') return false;
    for (int row = x - 1; row >= 0; row--) {
      if (++c < n && grid.get(row).charAt(c) == 'Q') return false;
      if (--d >= 0 && grid.get(row).charAt(d) == 'Q') return false;
    }
    return true;
  }
  ...
  public static class ChessGrid {
    ...
    public ChessGrid(List<String> grid) {
      ...
      _grid = new int[_N][_N];
      for (int i = 0; i < _N; i++)
        for (int j = 0; j < _N; j++) _grid[i][j] = grid.get(i).charAt(j) == 'Q' ? 1 : 0;
    }
    ...
    public ChessGrid rotateBy90() {
      ...
      for (int i = 0; i < _N; i++)
        for (int j = 0; j < _N; j++) rotatedGrid[i][j] = _grid[_N - j - 1][i];
