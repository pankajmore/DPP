...
public class LineMostPoints {
  /**
   * Time : O(n^2) assuming a good hash function Space : O(n)
   *
  ...
  public static Line getLineWithMostPoints(List<Point> points) {
    ...
    for (int i = 0; i < points.size(); i++) {
      ...
      for (int j = i + 1; j < points.size(); j++) {
        ...
        if (!lineToPoints.containsKey(l)) lineToPoints.put(l, new HashSet<>());
  ...
  public static class Line {
    ...
    @Override
    public String toString() {
      if (this.m == Double.MAX_VALUE) return "x = " + c;
      if (this.m == 0) return "y = " + c;
      return "y = " + m + "x + " + c;
    }
  }
}
