...
public class SudokuSolve {
    ...
    private static boolean solvePartialSudoku(List<List<Integer>> partialAssignment, int i, int j) {
        ...
        if (partialAssignment.get(i).get(j) != EMPTY_CELL) return solvePartialSudoku(partialAssignment, i, j + 1);
    ...
    private static boolean checkPositionWithVal(List<List<Integer>> partialAssignment, int i, int j, int val) {
        for (Integer col : partialAssignment.get(i)) if (col.equals(val)) return false;
        for (List<Integer> row : partialAssignment)
            if (row.get(j).equals(val)) return false;
        int regionSize = (int) Math.sqrt(partialAssignment.size());
        int I = i / regionSize, J = j / regionSize;
        for (int a = 0; a < regionSize; a++)
            for (int b = 0; b < regionSize; b++)
                if (partialAssignment.get(regionSize * I + a).get(regionSize * J + b).equals(val)) return false;
        return true;
    }
}
