...
public class ThreeSum {
  /**
   * Give an array of n numbers, find if there exist 3 (possibly repeated) numbers that sum to k
  ...
  public static boolean slow(Integer[] A, int k) {
    ...
    for (int x : A) for (int y : A) if (set.contains(k - x - y)) return true;
    return false;
  }
    ...
    for (Integer x : A) {
      for (Integer y : A) {
        ...
        if (t >= 0 && t < A.length) return Arrays.asList(x, y, k - x - y);
          ...
          map.put(k - x - y - A[A.length - 1], Arrays.asList(x, y, k - x - y));
        else {
