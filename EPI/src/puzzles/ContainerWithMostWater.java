...
public class ContainerWithMostWater {
   ...
   * Time : O(n) Space : O(1)
   *
   * <p>Invariant Property : If A[0] < A[n-1], for all x area(0,x) <= area(0,n-1), we can discard
   * A[0]. If A[0] > A[n-1], for all x area(x,n-1) <= area(0,n-1), we can discard A[n-1] If A[0] ==
   * A[n-1], we can discard both
   *
   * @param heights the list of heights of vertical lines
   * @return the maximum amount of water that can be filled between and two vertical lines
   */
