...
public class PermuteArray {
    /**
     * Permute the elements of A by applying the permutation sequence perm
     * Time: O(n)
     * Space: O(1)
     *
    ...
    public static void permute(List<Integer> A, List<Integer> perm) {
        int N = A.size();
        for (int i = 0; i < N; i++) if (perm.get(i) >= 0 /* not visited */) cyclePermutation(A, perm, i);
        for (int i = 0; i < N; i++) perm.set(i, perm.get(i) + N); /* reset permutation list to its original values */
    }
