...
/** Created by Pankaj on 1/11/16. Various ways to compute the powerSet of a set */
public class PowerSet {
  ...
  /**
   * Prints all the subsets of A Time : O(N*2^N) Space : O(N)
   *
   * @param A a list of size N
   */
  ...
  /**
   * Generate all subsets of A of size k Time : O(N^K) Space : O(N)
   *
  ...
  private static void subsetsOfFixedSizeHelper(
      List<Integer> A, List<Boolean> choosen, List<Integer> subset, int k) {
    if (k == 0) {
