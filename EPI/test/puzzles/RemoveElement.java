...
public class RemoveElement {
  /**
   * Deletes every instance of elem and shifts the remaining elements to left. The remaining
   * elements are treated as "dirty"
   *
  ...
  public static <T> int removeElement(T[] arr, T elem) {
    int j = 0;
    for (int i = 0; i < arr.length; i++) if (arr[i] != elem) arr[j++] = arr[i];
    return j;
  }
}
