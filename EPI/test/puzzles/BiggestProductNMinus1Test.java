...
public class BiggestProductNMinus1Test {
    ...
    public void compareWithBruteForce(@ForAll List<Double> A) {
        if (A.isEmpty())
            assert findBiggestProduct(A) == findBiggestProduct1(A) && findBiggestProduct(A) == findBiggestProduct2(A);
        else {
            assert Math.abs(findBiggestProduct(A) - findBiggestProduct1(A)) < 1E-7 * findBiggestProduct(A);
            assert Math.abs(findBiggestProduct(A) - findBiggestProduct2(A)) < 1E-7 * findBiggestProduct(A);
        }
    }
}