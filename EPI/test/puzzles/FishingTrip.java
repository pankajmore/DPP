...
public class FishingTrip {
  /**
   * The fisherman can only go right or down. Time : O(mn) Space : O(n)
   *
  ...
  /**
   * The fisherman can only go right or down. Time : O(mn) Space : O(mn)
   *
   * @param lake a grid of size m x n, each cell -> non-negative fish value
  ...
  public static List<Point> maximumFishPath(List<List<Integer>> lake) {
    ...
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < n; j++) {
        dp[i][j] =
            Math.max(i == 0 ? 0 : dp[i - 1][j], j == 0 ? 0 : dp[i][j - 1]) + lake.get(i).get(j);
      }
    }
